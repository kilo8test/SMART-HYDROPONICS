...
class _AuthVerificationScreenState extends State<AuthVerificationScreen>
  ...
  Future<void> _startVerificationProcess() async {
    try {
      ...
      print('🔐 LOGIN VERIFICATION for: $userEmail');
      print('   - userRole from database: $userRole');
      print('   - userName: $userName');

      // Check if user exists in user_profiles
      if (userRole == null) {
        print('   - User not in user_profiles, checking admin_requests...');
        // User not in user_profiles - check their admin request status
        final userRequest = await AdminService.getUserAdminRequest(response.user!.id);

        if (userRequest != null) {
          final status = userRequest['status'];
          print('   - Found admin request with status: $status');

          if (status == 'pending') {
            // Pending admin request
            print('   - ✅ Showing pending approval dialog');
            setState(() {
              _userRole = 'pending_admin';
              _userName = userName;
            });
            ...
            print('   - ❌ Showing rejected admin dialog');
            _showRejectedAdminDialog();
            return;
          } else if (status == 'approved') {
            // This shouldn't happen - approved admins should be in user_profiles
            print('   - ⚠️ Approved admin not in user_profiles - this is an error');
            _showErrorDialog('Account configuration error. Please contact administrator.');
            return;
          }
        } else {
          // No request found and not in user_profiles - this shouldn't happen for valid users
          print('   - ❌ No admin request found - showing rejected dialog');
          _showRejectedAdminDialog();
          return;
        }
      } else {
        print('   - ✅ User found in user_profiles with role: $userRole');
      }
  ...
  void _showRejectedAdminDialog() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
          'Your admin account request has been rejected by an administrator. Your account has been permanently removed and you cannot access the application anymore.',
          style: GoogleFonts.inter(
            color: Colors.grey[700],
          ),
        ),
  ...
  void _navigateToDashboard() {
    final routeName = _userRole == 'admin' ? AppRoutes.adminDashboard : AppRoutes.userDashboard;
    print('🎯 Navigating to: $routeName for role: $_userRole');
